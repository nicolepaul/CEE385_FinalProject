function [varargout] = defaultAntialias(varargin)
...
%
%   Usage:
%     defaultAntialias: Renders an anti-aliased version of the current figure.
%
%     defaultAntialias(K): Sets the supersampling factor to K. Using a 
%     higher K yields better rendering but takes longer time. If K is 
%     omitted, it defaults to 4. It may be useful to run e.g. defaultAntialias(2) to 
%     make a low-quality rendering as a first try, because it is a lot 
%     faster than the default.
%
%     defaultAntialias([K D]): Sets supersampling factor to K but downsamples the 
...
%
%     defaultAntialias('publish'): An experimental parameter, useful for publishing 
...
%        defaultAntialias('publish');  % Render an anti-aliased version
%
%     Then run:
%        publish test.m;
%        showdemo test;
...
elseif length(varargin) == 3
    ...
    error('Wrong syntax, run: help defaultAntialias');
end
...
if ~strcmp(self.figmode,'lazyupdate');
    tempfile = 'defaultAntialias_temp_screendump.png';
    self.source_fig = gcf;
...
if strcmp(self.figmode,'figure');
    ...
    self.defaultAntialias_figure = figure;
    fig = self.defaultAntialias_figure;
